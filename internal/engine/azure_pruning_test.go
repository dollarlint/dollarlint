...
func TestAzureARMResourcePruningDoesNotKeepPrefixedChildDefinitions(t *testing.T) {
	server := newAzureARMPrefixFixtureServer(t)
	defer server.Close()
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "azuredeploy.json"), azureARMPrefixFixtureTemplate(server.URL))
	...
	result, err := Lint(context.Background(), Options{Root: dir, Config: cfg})
	if err != nil {
		t.Fatalf("Lint prefixed child fixture: %v", err)
	}
	if !result.HasIssues() {
		t.Fatalf("expected invalid storage account issue")
	}
	var sawKind bool
	for _, issue := range result.Issues {
		if strings.Contains(issue.Message, "blobServices") || issue.InstanceLocation == "/resources/0/type" || issue.InstanceLocation == "/resources/0/name" {
			t.Fatalf("prefixed child resource definition was not pruned: %+v", result.Issues)
		}
		if issue.Keyword == "required" && issue.Property == "kind" {
			sawKind = true
		}
	}
	if !sawKind {
		t.Fatalf("expected storage account branch issue, got %+v", result.Issues)
	}
}
...
func TestAzureARMPruningSchemaEdges(t *testing.T) {
	...
	if azureARMRefAllowed("https://example.com/schemas/2023-01-01/Microsoft.Good.json#/resourceDefinitions/widgets_child", refs, "") {
		t.Fatalf("resource definition prefix should not be allowed")
	}
	if got := azureARMRefPart("https://example.com/schemas/2023-01-01/Microsoft.Good.json#/resourceDefinitions/widgets"); got != "/schemas/2023-01-01/Microsoft.Good.json#/resourceDefinitions/widgets" {
		t.Fatalf("ref part = %q", got)
	}
...
func newAzureARMPrefixFixtureServer(t *testing.T) *httptest.Server {
	t.Helper()
	var server *httptest.Server
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/schemas/2019-04-01/deploymentTemplate.json":
			fmt.Fprint(w, azureARMPrefixFixtureRootSchema(server.URL))
		case "/schemas/common/autogeneratedResources.json":
			fmt.Fprint(w, azureARMPrefixFixtureAutogeneratedSchema(server.URL))
		case "/schemas/2023-01-01/Microsoft.Storage.json":
			fmt.Fprint(w, azureARMPrefixFixtureStorageSchema())
		default:
			http.NotFound(w, r)
		}
	}))
...
func azureARMPrefixFixtureTemplate(baseURL string) string {
	return fmt.Sprintf(`{
  "$schema": %q,
  "contentVersion": "1.0.0.0",
  "resources": [
...
func azureARMPrefixFixtureRootSchema(baseURL string) string {
	return fmt.Sprintf(`{
  "$schema": "http://json-schema.org/draft-04/schema#",
  "id": %q,
  "type": "object",
...
func azureARMPrefixFixtureAutogeneratedSchema(baseURL string) string {
	return fmt.Sprintf(`{
  "$schema": "http://json-schema.org/draft-04/schema#",
  "id": %q,
  "allOf": [
...
func azureARMPrefixFixtureStorageSchema() string {
	return `{
  "$schema": "http://json-schema.org/draft-04/schema#",
  "resourceDefinitions": {
    "storageAccounts": {
      "type": "object",
      "required": ["type", "apiVersion", "name", "kind"],
      "properties": {
