...
func TestRealWorldSafeOutputPolicyIssuesRequirePRAndLinker(t *testing.T) {
	...
	lock := `{"create_discussion":{"category":"agentic-product-testing"},"create_pull_request":{"allowed_files":["reports/agentic-product-testing/**"],"if_no_changes":"error"},"link_real_world_outputs":true,"created_pr_url":"${{ steps.outputs.url }}","shell(printf)":true,"shell(safeoutputs:*)":true,"prompt":"should be merged in order to retain @agorischek","mentions":{"allowed":["agorischek"]}}`
	...
	if issues := realWorldSafeOutputPolicyIssues(source, lock); len(issues) != 7 {
		t.Fatalf("issues = %+v, want missing PR policy, linker, dispatch inputs, Discussion retention text, Discussion category, owner mention, and allowed files", issues)
	}
}

func TestRealWorldSafeOutputPolicyIssuesRequirePrintfForSafeoutputsCLI(t *testing.T) {
	source := "mentions:\n  allowed: [agorischek]\ncreate-discussion:\n  category: agentic-product-testing\ncreate-pull-request:\n  if-no-changes: error\n  allowed-files:\n    - reports/agentic-product-testing/**\njobs:\n  link-real-world-outputs:\n${{ github.event.inputs.max_repos }}\n${{ github.event.inputs.candidate_repos }}\nshould be merged in order to retain\n@agorischek\n"
	lock := `{"create_discussion":{"category":"agentic-product-testing"},"create_pull_request":{"allowed_files":["reports/agentic-product-testing/**"],"if_no_changes":"error"},"link_real_world_outputs":true,"created_pr_url":"${{ steps.outputs.url }}","shell(safeoutputs:*)":true,"prompt":"should be merged in order to retain @agorischek","mentions":{"allowed":["agorischek"]}}`
...
func TestRealWorldSafeOutputPolicyIssuesRejectPackageManagerShell(t *testing.T) {
	...
	lock := `{"create_discussion":{"category":"agentic-product-testing"},"create_pull_request":{"allowed_files":["reports/agentic-product-testing/**"],"if_no_changes":"error"},"link_real_world_outputs":true,"created_pr_url":"${{ steps.outputs.url }}","shell(printf)":true,"shell(npm ci --ignore-scripts)":true,"shell(safeoutputs:*)":true,"prompt":"should be merged in order to retain @agorischek","mentions":{"allowed":["agorischek"]}}`
	issues := realWorldSafeOutputPolicyIssues(source, lock)
