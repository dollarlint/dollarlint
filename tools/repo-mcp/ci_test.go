...
func TestRealWorldSafeOutputPolicyIssuesRequirePRAndLinker(t *testing.T) {
	...
	lock := `{"create_discussion":{"category":"agentic-product-testing"},"create_pull_request":{"if_no_changes":"error"},"link_real_world_outputs":true,"created_pr_url":"${{ steps.outputs.url }}","shell(printf)":true,"shell(safeoutputs:*)":true,"prompt":"should be merged in order to retain"}`
	...
	if issues := realWorldSafeOutputPolicyIssues(source, lock); len(issues) != 5 {
		t.Fatalf("issues = %+v, want missing PR policy, linker, dispatch inputs, Discussion retention text, and Discussion category", issues)
	}
}

func TestRealWorldSafeOutputPolicyIssuesRequirePrintfForSafeoutputsCLI(t *testing.T) {
	source := "create-discussion:\n  category: agentic-product-testing\ncreate-pull-request:\n  if-no-changes: error\njobs:\n  link-real-world-outputs:\n${{ github.event.inputs.max_repos }}\n${{ github.event.inputs.candidate_repos }}\nshould be merged in order to retain\n"
	lock := `{"create_discussion":{"category":"agentic-product-testing"},"create_pull_request":{"if_no_changes":"error"},"link_real_world_outputs":true,"created_pr_url":"${{ steps.outputs.url }}","shell(safeoutputs:*)":true,"prompt":"should be merged in order to retain"}`
